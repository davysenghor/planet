...
include('table_html_reservation.php');
...
function enregistrement_reservation() {
...
inputs.each(function (index, el) {
	...
	//alert(inputs_saisies);
$.ajax({
  method: "POST",
  url: "enregistrement_reservation.php",
  //Donnees envoyees par la methode Post
  
  data: {	
  Date_Mission: inputs_saisies[0], 
//Date_enregistrement: inputs_saisies[1]
			Debut: inputs_saisies[1],
			Fin: inputs_saisies[2],
			Langues:inputs_saisies[3],
			Interprete: inputs_saisies[4],
			Personne_Demandeur: inputs_saisies[5],
			Structure_Demandeur:inputs_saisies[6],
			Type_prestation: inputs_saisies[7],
			Statut_avancement: inputs_saisies[8],
			Duree_Heure: inputs_saisies[9],
			Duree_Minute: inputs_saisies[10],
			Num_Devis: inputs_saisies[11]
			}
...
//
	save_Button.on('click',enregistrement_reservation);

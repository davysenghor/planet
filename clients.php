...
include('form_reserv_avec_repeteur_clients.php');
echo '</div>'; 
...
include('table_html_reservation_clients.php');
...
// affichage table 
 var dataTable = $('#example_client').DataTable({
...
function enregistrement_client() {
...
$.ajax({
  method: "POST",
  url: "enregistrement_client.php",
  //Donnees envoyees par la methode Post
  data: {	PRENOM: inputs_saisies[0], 
			NOM : inputs_saisies[1],
			STRUCTURE: inputs_saisies[2],
			EMAIL: inputs_saisies[3],
			TELEPHONE: inputs_saisies[4]
			}
...
.done(function( les_prints_de_enreg_reserv ) {
   ...
   $('#example_client').DataTable().ajax.reload();
...
//
	save_Button.on('click',enregistrement_client);



	
 $('#example_client').on('draw.dt', function(){
/* 
...
*/  $('#example_client').Tabledit({
   url:'action_clients.php',
   dataType:'json',
   
   columns:{
    identifier : [0, 'ID'],
    editable:[[1, 'PRENOM'], [2, 'NOM'], [3,'STRUCTURE'],[4,'EMAIL'],[5,'TELEPHONE']] // colonnes éditables
   },
